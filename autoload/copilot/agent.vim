if exists('g:autoloaded_copilot_agent')  | finish  | endif
...
if !exists('s:instances')  | let s:instances = {}  | endif
...
function! s:AgentClose() dict abort
    if !has_key(self, 'job')  | return  | endif
...
let s:chansend = function( 'chansend' )
...
function! s:RequestAwait() dict abort
    call self.Wait()
    if has_key(self, 'result')  | return self.result  | endif
...
if !exists('s:id')  | let s:id = 0  | endif

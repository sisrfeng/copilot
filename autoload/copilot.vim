...
"\ setting
    ...
    "\     echom "s:config_root 是: "   s:config_root
    "\ endif
            "\ 在/home/wf/dotF/cfg/nvim/comE.vim里设置
...
"\ functions
        ...
        try
          ...
          \ )
            if response.status == 466
                return "Server error:  " . substitute(response.body, "\n$", '', '')
            en
    ...
    fun! s:commands.feedback(opts) abort
        ...
        let browser = copilot#Browser()
        if len(browser)
            call copilot#job#Stream(
                \ browser + [s:feedback_url],
                \ v:null,
                \ v:null,
                \ v:null,
               \ )
